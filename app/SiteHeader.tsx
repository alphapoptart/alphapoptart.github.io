...
export type SiteHeaderPage = 'home' | 'pricing' | 'card' | 'request';
...
export default function SiteHeader({ currentPage }: SiteHeaderProps): React.ReactNode {
  ...
  const nav = [
    ['Services', '/#services'], ['How it works', '/#process'], ['Pricing', '/pricing/'], ['Get help', '/request/'], ['Digital card', '/card/'],
  ] as const;
  return <>
    ...
    <header className="site-header">
      ...
      <nav className="desktop-nav" aria-label="Primary navigation">{nav.map(([label, href]) => <Link key={label} href={href} aria-current={(currentPage === 'pricing' && label === 'Pricing') || (currentPage === 'card' && label === 'Digital card') || (currentPage === 'request' && label === 'Get help') ? 'page' : undefined}>{label}</Link>)}</nav>
